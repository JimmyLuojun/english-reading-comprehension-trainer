...
version: v3
reason: Adds the inference error layer for implied relations and authorial stance.
---

# Sentence Analysis Prompt (Diagnosis Mode v3)
...
6. If the translation is correct, use `diagnosed_error_types: []`, include one `diagnosis_evidence` item with `"error_type": "OK"`, and make `blocking_point` a concise positive confirmation of the key structure preserved.
...
8. `blocking_point` must be grounded in `diagnosis_evidence`; do not introduce a new unsupported issue.
9. Use I01 for an unstated cause, contrast, concession, or condition relation across clauses or nearby sentences; use I02 for hedging, irony, evaluative wording, or authorial stance read as neutral or opposite.
10. `takeaway_suggestion` must be one Chinese sentence in this exact pattern: `遇到 [结构/搭配]，先检查 [动作]，否则易犯 [错误码]。`
11. If there are diagnosed errors, the error code used in `takeaway_suggestion` must be one of `diagnosed_error_types`. If the translation is correct, use `OK` in the final slot.
12. `confidence` is a float in [0.0, 1.0].
