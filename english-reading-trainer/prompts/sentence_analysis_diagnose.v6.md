...
version: v6
reason: Adds correct-path reinforcement for correct translations and structure attempts.
---

# Sentence Analysis Prompt (Diagnosis Mode v6)
...
18. Output `structure_feedback` only when USER STRUCTURE ATTEMPT is not `(none)`.
19. Every item in `structure_feedback.missed_or_wrong` must include one `error_code` from the Structure Feedback Code List.
20. `correct_highlights` must list 1-3 concrete choices the learner got right. If the attempt is not fully correct, include only the correct parts that should be preserved; use `[]` only when there is no correct structural claim.
21. If the structure attempt is fully correct, set `is_correct: true` and `missed_or_wrong: []`, still provide `corrected_structure` as the reference structure, and make `correct_highlights` the primary feedback.
22. When `structure_feedback.is_correct` is true, `next_check` must be a reusable self-check question, not a statement. Example: `遇到名词后的 -ing 短语时，我是否先确认句子已有真正的谓语？`
23. When `structure_feedback.is_correct` is true, `takeaway_suggestion` must use the positive reinforcement pattern from rule 16, even if the translation also has no diagnosed errors.
...
```json
{
  ...
  "structure_feedback": {
    ...
    "missed_or_wrong": [
      {
        "error_code": "G02",
        "learner_claim": "underpinning evolutionary psychology is the main verb phrase",
        "correction": "underpinning evolutionary psychology modifies consensus",
        "reason": "The finite main verb is shaped; underpinning is a participial modifier attached to consensus."
      }
    ],
    "correct_highlights": [],
    "corrected_structure": "Subject: The orthodox consensus underpinning evolutionary psychology; predicate: shaped policy.",
    "why_it_matters_for_translation": "Misreading the modifier as the predicate hides the true action 'shaped policy'.",
    "next_check": "Before translating -ing after a noun, have I checked whether the sentence already has a finite main verb?"
  }
}
```
