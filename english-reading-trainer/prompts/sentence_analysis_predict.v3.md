...
# Sentence Analysis Prompt (Prediction Mode v3)
...
Use inference codes only when the words and grammar are readable but the
likely blocker is an implied relation, implicature, or authorial stance.
...
7. Use I01 for an unstated cause, contrast, concession, or condition relation across clauses or nearby sentences; use I02 for hedging, irony, evaluative wording, or authorial stance likely to be read as neutral or opposite.
8. `takeaway_suggestion` must be one Chinese sentence in this exact pattern: `遇到 [结构/搭配]，先检查 [动作]，否则易犯 [错误码]。`
9. The error code used in `takeaway_suggestion` must be one of `predicted_error_types`.
