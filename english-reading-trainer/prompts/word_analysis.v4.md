...
name: word_analysis
version: v4
reason: v4 — adds learner_note_check so the reader Word Analysis panel can evaluate the learner's own note while preserving the v3 Chinese meaning output.
---

# Word / Phrase / Collocation Analysis Prompt (v4 — Writer's Perspective + Chinese Meaning + Learner Note Check)
...
9. `learner_note_check` evaluates only LEARNER NOTE ABOUT THIS ITEM. Do not let the learner note override the target item, target sentence, or the normal meaning analysis.
10. If the learner note is `(none)` or empty, use `status: "not_provided"` and set `feedback` and `corrected_understanding` to empty strings.
11. If the learner note is too short or too vague to judge, use `status: "not_enough_information"` and explain briefly in Chinese.
12. If the learner note is correct, use `status: "correct"` and give concise Chinese confirmation. If partly right or wrong, use `partly_correct` or `incorrect`, and give a specific correction without harsh wording.
13. `morphology.root` and `morphology.family` may be empty if not applicable.
14. `predicted_error_types` must be a list of 1–2 codes; never empty, never more than 2.
15. `confidence` is a float in [0.0, 1.0].
