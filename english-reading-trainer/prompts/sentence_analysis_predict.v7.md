...
# Sentence Analysis Prompt (Prediction Mode v7)
...
15. `argument_role` must be exactly one of: `claim`, `evidence`, `example`, `counterargument`, `concession`, `conclusion`, `background`, `qualification`, `transition`, `unclear`.
16. Use CONTEXT to decide the local argument role when it is helpful, especially previous/next sentences around the target sentence.
17. Use `unclear` when the role cannot be determined from the target sentence and available context. Do not force a confident label.
18. `argument_role_reason` must explain the role in plain English, grounded in the target sentence and local context.
19. `argument_role_check` must give one concrete reading warning or habit. For example, do not treat evidence as the author's final conclusion; do not treat a concession as full agreement.
20. Output `structure_feedback` only when USER STRUCTURE ATTEMPT is not `(none)`.
21. Every item in `structure_feedback.missed_or_wrong` must include one `error_code` from the Structure Feedback Code List.
22. `correct_highlights` must list 1-3 concrete choices the learner got right. If the attempt is not fully correct, include only the correct parts that should be preserved; use `[]` only when there is no correct structural claim.
23. If the structure attempt is fully correct, set `is_correct: true` and `missed_or_wrong: []`, still provide `corrected_structure` as the reference structure, and make `correct_highlights` the primary feedback.
24. When `structure_feedback.is_correct` is true, `next_check` must be a reusable self-check question, not a statement. Example: `遇到名词后的从句时，我是否先确认它修饰哪个名词？`
25. When `structure_feedback.is_correct` is true, `takeaway_suggestion` must positively reinforce the current success in Chinese: `这次你做对了 [具体判断]；下次遇到 [结构]，继续先 [复现动作]。` Do not use “否则易犯” for a fully correct structure attempt.
...
```json
{
  ...
  "argument_role_reason": "The sentence reports a specific board action that can support a larger claim about how the report was received.",
  "argument_role_check": "Do not treat one supporting event as the author's final conclusion unless nearby context makes that turn explicit.",
