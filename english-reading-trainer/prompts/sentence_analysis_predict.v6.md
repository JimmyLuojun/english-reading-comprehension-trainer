...
version: v6
reason: Adds correct-path reinforcement for learner structure attempts.
---

# Sentence Analysis Prompt (Prediction Mode v6)
...
17. `correct_highlights` must list 1-3 concrete choices the learner got right. If the attempt is not fully correct, include only the correct parts that should be preserved; use `[]` only when there is no correct structural claim.
18. If the structure attempt is fully correct, set `is_correct: true` and `missed_or_wrong: []`, still provide `corrected_structure` as the reference structure, and make `correct_highlights` the primary feedback.
19. When `structure_feedback.is_correct` is true, `next_check` must be a reusable self-check question, not a statement. Example: `遇到名词后的从句时，我是否先确认它修饰哪个名词？`
20. When `structure_feedback.is_correct` is true, `takeaway_suggestion` must positively reinforce the current success in Chinese: `这次你做对了 [具体判断]；下次遇到 [结构]，继续先 [复现动作]。` Do not use “否则易犯” for a fully correct structure attempt.
...
```json
{
  ...
  "structure_feedback": {
    ...
    "next_check": "When a relative clause follows a noun, have I attached it to the nearest semantically valid noun?"
  }
}
```
