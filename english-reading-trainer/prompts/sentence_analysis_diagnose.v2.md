...
version: v2
reason: Adds minimal recursive-learning fields grounded in the learner's translation evidence.
---

# Sentence Analysis Prompt (Diagnosis Mode v2)
...
9. `takeaway_suggestion` must be one Chinese sentence in this exact pattern: `遇到 [结构/搭配]，先检查 [动作]，否则易犯 [错误码]。`
10. If there are diagnosed errors, the error code used in `takeaway_suggestion` must be one of `diagnosed_error_types`. If the translation is correct, use `OK` in the final slot.
