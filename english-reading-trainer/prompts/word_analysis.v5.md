...
name: word_analysis
version: v5
reason: Adds role_in_sentence so word analysis returns from local vocabulary detail to whole-sentence comprehension.
---

# Word / Phrase / Collocation Analysis Prompt (v5 — Writer's Perspective + Chinese Meaning + Learner Note Check + Sentence Role)
...
Use a minimal recursive reading loop: explain the local item, then connect it back to the whole sentence.
...
LEARNER NOTE ABOUT THIS ITEM (may be empty):
{{ learner_note }}
...
```json
{
  ...
  "role_in_sentence": "<2-3 sentences: the item's syntactic/semantic role in this sentence and how misunderstanding it would distort the whole sentence>",
  ...
  "learner_note_check": {
    ...
    "feedback": "<中文反馈：评价 learner note 是否准确，1-2 句；没有 learner note 时为空字符串>",
    "corrected_understanding": "<中文修正：如果 learner note 有偏差，给出更准确理解；没有偏差或没有 note 时为空字符串>"
  },
  "morphology": {
...
6. `role_in_sentence` must connect the local item back to whole-sentence comprehension. For phrases/collocations, include the pattern inside this field when it matters.
7. `register`: choose the single best-fit register. "academic" for formal scholarly language; "technical" for domain-specific jargon; "formal" for elevated but non-academic prose; "literary" for poetic/expressive writing; "neutral" for everyday unmarked usage; "colloquial" for informal speech.
8. `why_this_word`: focus on the *writerly motivation* — connotation, register precision, degree semantics, or collocation restriction that makes the item the best choice here. End with a concrete "if you wrote X instead, you would lose Y" sentence.
9. `vs_simpler`: 1-3 entries. Each `simpler` word must be a plainer, more common alternative. The `difference` should echo the contrast already explained in `why_this_word`.
10. `learner_note_check` evaluates only LEARNER NOTE ABOUT THIS ITEM. Do not let the learner note override the target item, target sentence, or the normal meaning analysis.
11. If the learner note is `(none)` or empty, use `status: "not_provided"` and set `feedback` and `corrected_understanding` to empty strings.
12. If the learner note is too short or too vague to judge, use `status: "not_enough_information"` and explain briefly in Chinese.
13. If the learner note is correct, use `status: "correct"` and give concise Chinese confirmation. If partly right or wrong, use `partly_correct` or `incorrect`, and give a specific correction without harsh wording.
14. `morphology.root` and `morphology.family` may be empty if not applicable.
15. `predicted_error_types` must be a list of 1-2 codes; never empty, never more than 2.
...
```json
{
  ...
  "role_in_sentence": "It modifies 'graphical interfaces' and tells the reader that the interfaces existed but were still immature. If the learner reads it merely as 'basic', the sentence loses the idea that these interfaces were early-stage forms that later developed.",
  "register": "academic",
  ...
  "learner_note_check": {
    "status": "correct",
    "feedback": "你的理解是正确的：这里强调的是处在初级阶段，而且功能还不完善。",
