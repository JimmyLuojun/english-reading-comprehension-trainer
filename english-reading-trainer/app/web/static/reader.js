    ...
    (() => {
      ...
      let toolbarEditorPlacementSettled = false;
      ...
      function hideToolbar(options = {}) {
        ...
        toolbar.hidden = true;
        reader.style.paddingBottom = "";
        toolbarEditorPlacementSettled = false;
      ...
      function positionToolbar(anchor) {
        ...
        requestAnimationFrame(() => {
          ...
          const editorOpen = translationEditorOpen || structureEditorOpen;
          const editorScrollSpace = `${toolbarRect.height + viewportPadding * 2}px`;
          if (editorOpen && reader.style.paddingBottom !== editorScrollSpace) {
            reader.style.paddingBottom = editorScrollSpace;
            requestAnimationFrame(() => {
          ...
          if (!editorOpen && reader.style.paddingBottom) reader.style.paddingBottom = "";
          if (
            editorOpen
            && toolbarRect.height > availableAbove
            && toolbarRect.height > availableBelow
          ) {
            const desiredAnchorEdge = availableBelow >= availableAbove
              ? maxViewportTop - gap
              : toolbarRect.height + gap + viewportPadding;
            const currentAnchorEdge = availableBelow >= availableAbove
              ? anchor.bottom
              : anchor.top;
            const scrollBefore = window.scrollY;
            window.scrollBy(0, currentAnchorEdge - desiredAnchorEdge);
            if (window.scrollY !== scrollBefore && activeSentenceId) {
              requestAnimationFrame(() => {
                const sentence = document.getElementById(`sentence-${activeSentenceId}`);
                if (sentence) positionToolbar(sentence.getBoundingClientRect());
              });
          ...
          if (editorOpen && !toolbarEditorPlacementSettled) {
            toolbarEditorPlacementSettled = true;
            requestAnimationFrame(() => {
              const sentence = document.getElementById(`sentence-${activeSentenceId}`);
              if (sentence) positionToolbar(sentence.getBoundingClientRect());
            });
