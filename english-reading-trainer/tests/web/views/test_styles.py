...
def test_css_contains_reader_and_popover_selectors() -> None:
    ...
    assert "#toolbar-word-note {\n      grid-area: note;" in css
    assert "#toolbar-word-save { grid-area: save; }" in css
    ...
    assert ".chapter-row-readable" in css
    assert ".chapter-row-readable:focus-within" in css
    assert ".chapter-row-link::after" in css
