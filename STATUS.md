...
- Sentence Analysis now surfaces editable `Your translation` and `Takeaway` fields in the reader panel, with Takeaway stored in the existing `sentence_cards.user_note` column. Cards shows a Takeaway column, sentence Text links deep-link back to `/read/{book_id}?chapter={idx}&sentence_id={id}&panel=analysis`, and the Books page exposes explicit Continue reading from existing per-book reader progress.
- Dashboard metric cards now act as navigation shortcuts: Books opens the book list, Sentence cards and Word cards jump to anchored Cards sections, and Due now opens Review. The top Books nav always returns to the full book list, while Reader provides separate All books and Chapters buttons.
