...
- `I01`：分句或相邻句之间存在未明说的因果、转折、让步、条件等关系，读者把两部分当成无关事实，或推错关系。
- `I02`：作者立场、含蓄评价、保留语气或反讽信号被读成中性或相反断言，例如 `so-called`、`may`、`in theory`、`some`。

**理由**：这套分类参考 L2 阅读研究中常见的"句法/词汇/篇章"三分，并补入高级阅读中常见的推理失败。每个标签必须能在卡片上可操作、可统计、可对应到具体的复习题型。
...
```text
X00 其他（不属于上述四层；必须在 diagnosis_evidence 中说明）
```

- 兜底类**不计入** §7.5 的"top-3 高频错因"优先级，避免被 LLM 滥用为偷懒选项。
- 当 `X00` 在一段时间内反复出现，由开发者人工审视是否需要扩枚举。
- 当前 `X00` 只是文档与 reader 显示层兜底标签，尚未种入 `db_models.ERROR_TYPES`，也不在 prompt 封闭错误码清单中。
