...
- Web Import 提供 `Auto / Book / Article / Excerpt`；Auto 按渠道/格式建议语义类型，TXT/Markdown/PDF file 保持 `unclassified`。所有详细见 `docs/features/library-items.md`。
...
- 原 URL 保存在 `books.source_uri`；语义类型 Auto 为 `article`，解析格式仍为 `txt`。
